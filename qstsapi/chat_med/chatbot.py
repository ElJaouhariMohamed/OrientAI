...
class chatbot() :
    ...
    def prepare_tokenz(self) :
        ...
        with open(formatted_path+'/qstsapi/chat_med/tokinzers/tokenizers_ques.pickle', 'rb') as handle:
            self.tokenizer_quest = pickle.load(handle)

        with open(formatted_path+'/qstsapi/chat_med/tokinzers/tokenizers_answ.pickle', 'rb') as handle:
    ...
    def prepare_glove(self) :
        ...
        embeddings_index = {}
        if not os.path.exists(formatted_path + '/qstsapi/chat_med/glove.6B.300d.txt') :

            import importlib

            module_name = "zipfile36"
            ...
            try:
                ...
                #print(f"The module '{module_name}' has been installed.")
            ...
            with open(formatted_path +'/qstsapi/chat_med/glove.6B.zip', 'wb') as f:
                f.write(response.content)

            with zipfile.ZipFile(formatted_path +'/qstsapi/chat_med/glove.6B.zip', 'r') as zip_ref:
                # Extract all files to a specified directory
                zip_ref.extractall(formatted_path+"/qstsapi/chat_med")

            l = ["glove.6B.200d.txt","glove.6B.100d.txt","glove.6B.50d.txt"]

            for i in l :
                os.remove(formatted_path+"/qstsapi/chat_med/"+i)

        with open(formatted_path +'/qstsapi/chat_med/glove.6B.300d.txt', 'r',encoding='utf-8') as f:
            for line in f:
                values = line.split()
                word = values[0]
        ...
        self.embedding_matrix_que = np.zeros((self.vocab_quest, 300)) # the first value is vocab_ques
        ...
        self.embedding_matrix_answ = np.zeros((self.vocab_answ, 300)) # the second value is vocab_answ
    ...
    def prepare_models(self):
        ...
        self.encoder =  Encoder(self.vocab_quest,self.embedding_dim, self.units, self.BATCH_SIZE,self.embedding_matrix_que)
        sample_input = tf.ones((64, 19))
        sample_hidden = self.encoder.initialize_hidden_state()
        sample_output, sample_hidden = self.encoder(sample_input, sample_hidden)
        self.encoder.load_weights(formatted_path + "/qstsapi/chat_med/weights/encoder/encoder_512_ep450_bs64_cpu_100.h5")
        ...
        self.decoder =  Decoder(self.vocab_answ, self.embedding_dim, self.units, self.BATCH_SIZE,self.embedding_matrix_answ)
        self.decoder(tf.random.uniform((64, 1)),sample_hidden, sample_output)
        self.decoder.load_weights(formatted_path+"/qstsapi/chat_med/weights/decoder/decoder_512_ep450_bs64_cpu100.h5")
    ...
    def pipeline(self,text):
        ...
        text = re.sub(r"ecole national des sciences appliquées de khouribga", "ensa khouribga",  text)
        text = re.sub(r"\?", "?",  text)
    ...
    def evaluate(self,sentence):
        ...
        inputs = [self.tokenizer_quest.word_index[i] for i in sentence.split(" ")]
        ...
        dec_input = tf.expand_dims([self.tokenizer_answ.word_index['<start>']], 0)
